...
public class GizmoManager : MonoBehaviour
{
	...
	[SerializeField]
	GameObject palmCenterGizmo, POIGizmo;
	[SerializeField]
	GameObject flagHolderGizmo;
	[SerializeField]
	GameObject smoothingSliderControler;
    [SerializeField]
    GameObject gestureSmoothingSliderControler;
    [SerializeField]
	Text currentSmoothingValue;
    [SerializeField]
    Text currentGestureSmoothingValue;
    [SerializeField]
	...
	RectTransform palmCenterRectTransform, palmCenterFillAmmount, poiRectTransform, poiFillAmmount;
	...
	void DisplayInformationAfterManoMotionProcessFrame()
	{
		...
		DisplayPalmCenter(trackingInfo, gestureInfo, warning);
        ...
        DisplayCurrentGesureSmoothingValue(session);
    ...
    /// <summary>
	/// Displays in text value the current gesture smoothing value of the session
	/// </summary>
	/// <param name="session">Session.</param>
    ...
    void DisplayPalmCenter(TrackingInfo tracking_info, GestureInfo gesture, Warning warning) // AQUI
	{
		if (ShowPalmCenter)
		{
	...
	void DisplayPOI(GestureInfo gesture, Warning warning, TrackingInfo trackingInfo)
	{
		...
		if (ShowPOI)
		{
			...
			if (warning != Warning.WARNING_HAND_NOT_FOUND && isPinchWellDetected)
			{
				...
				float smoothing = 1 - ManomotionManager.Instance.Manomotion_Session.smoothing_controller;              
                poiRectTransform.position =ManoUtils.Instance.CalculateScreenPosition(trackingInfo.poi, trackingInfo.depth_estimation);
                float newFillAmmount = 1 - ((int)(gesture.state / 6) * 0.25f);
